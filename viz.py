...
df.drop(labels=['even', 'amount', 'chance', 'front', 'happen', 'line', 'man', 'name', 'number', 'part', 'plan', 'point', 'post', 'power', 'reason', 'result', 'shit', 'stuff', 'team', 'time', 'well', 'while',], axis=0, inplace=True)
df.drop(labels=['aback'], axis=1, inplace=True)

# selects a small subset of all verbs
# a different interesting subset happens when using .mean instead
df = df[df.max(axis=1) > 14000000]
...
Frequency of phrasal verbs (e.g., "find out", "get over") in Reddit comments, 2005-2019. Viz by /u/gregsadetsky
